# Assignment 4: Selection, Mastermind

### Deadline

6 October 2021 (Wednesday 23:59 pm)
...
- First, [accept the assignment on GitHub](https://classroom.github.com/a/YcHb1yJD)
...
- You should see the folder `as04-<github username>` in your home directory with the assignment skeleton inside.
- Inside that directory, you should see the following files:
    - `selection.c` and `mastermind.c` are the most important files.  They are the skeleton C code that you should edit to solve the assignment and the only files that you should change.
...
- Edit the files `selection.c` and `mastermind.c` to solve the corresponding question as described below.
...
The four files `selection.c` and `mastermind.c` will be uploaded to GitHub.  You can submit multiple times, but only the last submission will be graded.
