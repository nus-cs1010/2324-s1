...
Let's continue the discussion on efficiency on one of the fundamental problems in computing: how to look for something.  Given a list of items $L$ and query item $q$, we wish find if $q$ is in $L$.  If $q$ is in $L$, then we return the index of $q$.  Otherwise, we return -1.
...
In previous units, we have seen recursive, divide-and-conquer solution for `max`.  We can easily adapt it to search as well:
...
We then call this function with:

```C
search(list, 0, n-1, q);
```

In the code above, we first handle the base case where `i > j`, which means that the sublist `list[i]..list[j]` to search for `q` in, is empty.  In this
case, `q` is not found and we return -1.  If the list `list[i] .. list[j]` is not empty, we check the element in the middle if it is `q`.  If we have not yet find `q`, then we first search the left sublist.  If still not found, we search the right sublist. and return the result.

In the worst case, when `q` is not in the list, we still have to check through every element in the list.  So the running time is still $O(n)$.  It is no faster than searching with a for loop.
...
We have changed the function so that it only search either the left side or the right side, but never both.
...
As you will see in your upcoming assignment, exploiting underlying properties of inputs is a crucial weapon we have in speeding up our algorithm.
...
Instead of returning -1 if the query `q` is not found, modify the binary search algorithm in Problem 22.1 such that it returns either:
