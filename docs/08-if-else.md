...
When we think about writing conditionals, we have to _exhaustively reason about what are all the possible scenarios that could occur_.  In this example, we need to think about what are the possible relationships between `x` and `y` when we compare `x` an `y`.  There are actually three possibilities!
...
In CS1010, we following this rule: we do not write an `else` after a `return` statement, since it is redundant.
...
Nesting conditional operators is therefore not allowed in CS1010.
...
Draw the flowcharts for the three code snippets below.  Identify redundant comparisons (if any), in each of the snippet.
